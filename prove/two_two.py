...
map_ansatz_one_one=ansatz.map_ansatz('one_one',[[0,0],[0,1]],2)
...
for one_one_map_ansatz_term_first in map_ansatz_one_one:
    for one_one_map_ansatz_term_second in map_ansatz_one_one:
        ...
        second['coefficient']*=(-1)**len(second['index_based'][0][1])
...
for one_one_map_ansatz_term_third in map_ansatz_one_one:
    for one_one_map_ansatz_term_fourth in map_ansatz_one_one:
        ...
        fourth['coefficient']*=(-1)**len(fourth['index_based'][0][1])
...
for term in result_operation_A:
    for action_class in two_two_A_action_class:
        for position_based in action_class['position_based']:
            if term['position_based']==position_based:
                coefficient=term['coefficient']
                variable=term['id']
                two_two_condition[action_class['id']]=two_two_condition.get(action_class['id'],'')+f'+({coefficient})*{variable}'
                break
...
for key in two_two_condition:
    two_two_condition[key]+="-action_"+key

for term in result_operation_B:
    for action_class in two_two_B_action_class:
        for position_based in action_class['position_based']:
            if term['position_based']==position_based:
                coefficient=term['coefficient']
                variable=term['id']
                ...
                break
'''
for action_class,condition in two_two_condition.items():
    if not condition:
        two_two_condition[action]="0"
'''
two_two_condition_new={}      

for key,value in two_two_condition.items():
    expr=sp.Integer(0)
    bucket=[]
    ...
    if bucket:
        expr=sp.Add(expr, sp.Add(*bucket))
    two_two_condition_new.update({sp.Symbol(key):deepcopy(expr)})

two_two_condition=two_two_condition_new

BA_inear_combination=[]
CA_inear_combination=[]

for term in two_two_B_action_class:
    if len(term['index_based'][0][1])==2:
        if term['index_based'][0][1][0]==term['index_based'][0][1][1]:
            '''
            print("B")
            ...
            term['index_based'].pop(0)
            term['index_based'][1][0] = 1
            for term2 in two_two_A_action_class:
...
for term in two_two_C_action_class:
    for element_order, element in enumerate(term['index_based']):
        if len(element[1])==2:
            if element[1][0]==element[1][1]:
                index_based=deepcopy(term['index_based'])
                index_based.pop(element_order)
                for term2 in two_two_A_action_class:
                    if position_based_of_(index_based_reorganize(index_based)) in term2['position_based']:
...
solution,pivot_variable_list=linear_algebra.gauss_jordan_solution(
    ...
    ansatz.action_class('two_two_C',[[0,1],[0,1],[0,1]],4),
    [[0,[1,'+'],[2,'+']],
     [1,[0,'+'],[2,'+']],
     [2,[0,'+'],[1,'+']]]
    )
...
for pivot_variable in pivot_variable_list:
    for variable,coefficient in solution[pivot_variable][1].items():
        two_two_condition[variable]=two_two_condition.get(variable,sp.Integer(0))+coefficient*(two_two_condition.get(pivot_variable,sp.Integer(0)))
    two_two_condition.pop(pivot_variable, None)
