...
for zero_one_map_ansatz_term_first in ansatz.map_ansatz('zero_one',[[0,0],[1,1],[1,1]],2):
    for one_one_map_ansatz_term_first in map_ansatz_one_one:
        ...
        one_one['coefficient']*=(-1)**len(one_one['index_based'][0][1])
        ...
        if zero_one['eta']==0:
            if one_one['eta']==0:
                ...
                for symmetrization in [[1,2],[2,1]]:
                    ...
                    zero_one_index_based[0][1]+=one_one_index_based[0].pop(1)
            ...
            if one_one['eta']==0:
                ...
                one_one['index_based']=index_based_element_switch(one_one['index_based'],[1],[4])
        zero_one['index_based'][0][1]+=one_one['index_based'][0].pop(1)        
...
for left in one_two_A_left:
    for action_class in one_two_A_action_class:
        if position_based_of_(left['index_based']) in action_class['position_based']:
            one_two_condition[sp.Symbol(action_class['id'])]-=sp.sympify(left['coefficient'])

for left in one_two_C_left:
    for action_class in one_two_C_action_class:
        if position_based_of_(left['index_based']) in action_class['position_based'][:4]:
            one_two_condition[sp.Symbol(action_class['id'])]-=sp.sympify(left['coefficient'])
            continue
        if position_based_of_(left['index_based']) in action_class['position_based'][4:]:
            one_two_condition[sp.Symbol(action_class['id'])]+=sp.sympify(left['coefficient'])
    ...
    if len(term['index_based'][1][1])==2:
        if term['index_based'][1][1][0]==term['index_based'][1][1][1]:
            ...
            for term2 in one_two_A_action_class:
                ...
                CA_inear_combination.append(sp.sympify(term['id']))
            continue
